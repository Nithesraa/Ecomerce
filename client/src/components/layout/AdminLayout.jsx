  ...
  if (user?.role === 'SELLER' && user?.isVerifiedSeller === false) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        ...
        <h1 className="text-xl font-black uppercase tracking-tight text-black mb-2 text-center">Account Pending Verification</h1>
      ...
      <aside className="w-64 bg-white dark:bg-[#111111] border-r border-gray-200 dark:border-white/[0.05] flex flex-col fixed h-full z-20">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-white/[0.05]">
          <div className="flex items-center gap-3 px-4 py-3 mb-2">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-sm uppercase">
              {user?.name?.charAt(0) || 'U'}
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-bold text-black dark:text-white truncate">{user?.name}</span>
              <span className="text-sm text-gray-500 uppercase tracking-wider">{user?.role}</span>
            </div>
          </div>
          <button
            onClick={handleLogout}
